...
export class GuestController {
  ...
  getGuests = async (req: Request, res: Response) => {
    ...
    this.guestService.getGuests(paginationDto!)
  ...
  createGuest = (req: Request, res: Response) => {
    ...
    this.guestService.createGuest(createStudentDto!, req.body.user)
  ...
  updateGuest = (req: Request, res: Response) => {
    ...
    this.guestService.updateGuest(updateStudentDto!, req.body.user, parseInt(req.params.id))
      .then(guest => res.status(201).json(guest))
      .catch(error => this.handleError(error, res));
  ...
  deleteGuest = (req: Request, res: Response) => {
    ...
    this.guestService.deleteGuest(req.body.user, parseInt(req.params.id))
      .then(guest => res.status(201).json(guest))
      .catch(error => this.handleError(error, res));
