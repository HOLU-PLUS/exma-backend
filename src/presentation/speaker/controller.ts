...
export class SpeakerController {
  ...
  createSpeaker = (req: Request, res: Response) => {
    ...
    this.speakerService
      .createSpeaker(createTeacherDto!, req.body.user)
  ...
  updateSpeaker = (req: Request, res: Response) => {
    ...
    this.speakerService
      .updateSpeaker(updateTeacherDto!, req.body.user, parseInt(req.params.id))
      .then((speaker) => res.status(201).json(speaker))
      .catch((error) => this.handleError(error, res));
  };
  ...
  deleteSpeaker = (req: Request, res: Response) => {
    this.speakerService
      .deleteSpeaker(req.body.user, parseInt(req.params.id))
      .then((speaker) => res.status(201).json(speaker))
      .catch((error) => this.handleError(error, res));
  };
}
